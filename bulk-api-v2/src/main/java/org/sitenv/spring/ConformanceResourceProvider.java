...
public class ConformanceResourceProvider extends ServerConformanceProvider {
	...
	public Conformance getConformance(HttpServletRequest request) {
		...
		Rest rest = restList.get(0);

		RestSecurity restSecurity = new RestSecurity();

		ExtensionDt conformanceExtension = new ExtensionDt(false,
		...
		conformanceExtension
				.addUndeclaredExtension(new ExtensionDt(false, "register", new UriDt(uri + "/view/newuser.html")));

		restSecurity.addUndeclaredExtension(conformanceExtension);

		BoundCodeableConceptDt<RestfulSecurityServiceEnum> boundCodeableConceptDt = new BoundCodeableConceptDt<>(
