...
public class DeviceResourceProvider implements IResourceProvider {
	Logger log = (Logger) LoggerFactory.getLogger(DeviceResourceProvider.class);
	 ...
	 * @return
	 */
	public void getDeviceForBulkDataRequest(StringJoiner patients, Date start, File destDir, FhirContext ctx) {
		String fileName = "Device.ndjson";
		List<DafDevice> dafDeviceList = service.getDeviceForBulkData(patients,
				start);
		File ndJsonFile = new File(destDir.getAbsolutePath() + "/" + fileName);
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < dafDeviceList.size(); i++) {

			sb.append((ctx.newJsonParser()
					.encodeResourceToString(createDeviceObject(dafDeviceList.get(i)))))
					.append("\r\n");
		}
		try (FileWriter writer = new FileWriter(ndJsonFile, true); BufferedWriter bw = new BufferedWriter(writer)) {
			...
			log.error("error occured while writing to Device file", e);
		}
	}
