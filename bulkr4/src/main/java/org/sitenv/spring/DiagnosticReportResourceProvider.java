...
public class DiagnosticReportResourceProvider implements IResourceProvider {
	Logger log = (Logger) LoggerFactory.getLogger(DiagnosticReportResourceProvider.class);
     ...
     * @param destDir
     */
	public void getDiagnosticReportForBulkDataRequest(StringJoiner patients, Date start, File destDir, FhirContext ctx) {
		String fileName = "DiagnosticReport.ndjson";
		List<DafDiagnosticReport> dafDiagnosticReportList = service.getDiagnosticReportForBulkData(patients,
				start);
		File ndJsonFile = new File(destDir.getAbsolutePath() + "/" + fileName);
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < dafDiagnosticReportList.size(); i++) {

			sb.append((ctx.newJsonParser()
					.encodeResourceToString(createDiagnosticReportObject(dafDiagnosticReportList.get(i)))))
					.append("\r\n");
		}
		try (FileWriter writer = new FileWriter(ndJsonFile, true); BufferedWriter bw = new BufferedWriter(writer)) {
			...
			log.error("error occured while writing to DiagnosticReport file", e);
		}
	}
