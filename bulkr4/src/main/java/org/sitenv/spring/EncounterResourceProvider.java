...
public class EncounterResourceProvider implements IResourceProvider {
	Logger log = (Logger) LoggerFactory.getLogger(EncounterResourceProvider.class);
    ...
    public void getEncounterForBulkDataRequest(StringJoiner patients, Date start, FhirContext ctx,
			File destDir) {
    	String fileName = "Encounter.ndjson";
    	List<DafEncounter> dafEncounterList = service.getEncounterForBulkData(patients,
				start);
		File ndJsonFile = new File(destDir.getAbsolutePath() + "/" + fileName);
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < dafEncounterList.size(); i++) {

			sb.append((ctx.newJsonParser()
					.encodeResourceToString(createEncounterObject(dafEncounterList.get(i)))))
					.append("\r\n");
		}
		try (FileWriter writer = new FileWriter(ndJsonFile, true); BufferedWriter bw = new BufferedWriter(writer)) {
			...
			log.error("error occured while writing to Encounter file", e);
		}
	}
