...
public class GoalResourceProvider implements IResourceProvider {
	Logger log = (Logger) LoggerFactory.getLogger(GoalResourceProvider.class);
	 ...
	 * @return
	 */
	public void getGoalsForBulkDataRequest(StringJoiner patients, Date start, File destDir, FhirContext ctx) {
		String fileName = "Goal.ndjson";
		List<DafGoal> dafGoalList = service.getGoalForBulkData(patients, start);
		File ndJsonFile = new File(destDir.getAbsolutePath() + "/" + fileName);
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < dafGoalList.size(); i++) {

			sb.append((ctx.newJsonParser().encodeResourceToString(createGoalObject(dafGoalList.get(i)))))
					.append("\r\n");
		}
		try (FileWriter writer = new FileWriter(ndJsonFile, true); BufferedWriter bw = new BufferedWriter(writer)) {
			...
			log.error("error occured while writing to Goal file", e);
		}
	}
