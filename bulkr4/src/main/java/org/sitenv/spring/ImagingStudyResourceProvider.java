...
public class ImagingStudyResourceProvider implements IResourceProvider{
	Logger log = (Logger) LoggerFactory.getLogger(ImagingStudyResourceProvider.class);
     ...
     * @param destDir
     */
	public void getImagingStudyRequestDataRequest(StringJoiner patients, Date start, File destDir, FhirContext ctx) {
		String fileName = "ImagingStudy.ndjson";
		List<DafImagingStudy> dafImagingStudyList = service.getImagingStudyForBulkData(patients, start);
		File ndJsonFile = new File(destDir.getAbsolutePath() + "/" + fileName);
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < dafImagingStudyList.size(); i++) {

			sb.append(
					(ctx.newJsonParser().encodeResourceToString(createImagingStudyObject(dafImagingStudyList.get(i)))))
					.append("\r\n");
		}
		try (FileWriter writer = new FileWriter(ndJsonFile, true); BufferedWriter bw = new BufferedWriter(writer)) {
			...
			log.error("error occured while writing to ImagingStudy file", e);
		}
	}
