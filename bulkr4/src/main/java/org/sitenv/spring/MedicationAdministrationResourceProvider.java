...
public class MedicationAdministrationResourceProvider implements IResourceProvider {
	Logger log = (Logger) LoggerFactory.getLogger(MedicationAdministrationResourceProvider.class);
	 ...
	 * @param ctx
	 */
	public void getMedicationAdministrationForBulkDataRequest(StringJoiner patients, Date start, File destDir,
			FhirContext ctx) {
		String fileName = "MedicationAdministration.ndjson";
		List<DafMedicationAdministration> dafMedicationAdministrationList = service
				.getMedicationAdministrationForBulkData(patients, start);
		File ndJsonFile = new File(destDir.getAbsolutePath() + "/" + fileName);
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < dafMedicationAdministrationList.size(); i++) {

			sb.append((ctx.newJsonParser().encodeResourceToString(
					createMedicationAdministrationObject(dafMedicationAdministrationList.get(i))))).append("\r\n");
		}
		try (FileWriter writer = new FileWriter(ndJsonFile, true); BufferedWriter bw = new BufferedWriter(writer)) {
			...
			log.error("error occured while writing to MedicationAdministration file", e);
		}
	}
