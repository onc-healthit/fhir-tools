...
public class MedicationDispenseResourceProvider implements IResourceProvider {
	Logger log = (Logger) LoggerFactory.getLogger(MedicationDispenseResourceProvider.class);
	 ...
	 * @param ctx
	 */
	public void getMedicationDispenseForBulkDataRequest(StringJoiner patients, Date start, File destDir,
			FhirContext ctx) {
		String fileName = "MedicationDispense.ndjson";
		List<DafMedicationDispense> dafMedicationDispenseList = service.getMedicationDispenseForBulkData(patients,
				start);
		File ndJsonFile = new File(destDir.getAbsolutePath() + "/" + fileName);
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < dafMedicationDispenseList.size(); i++) {

			sb.append((ctx.newJsonParser()
					.encodeResourceToString(createMedicationDispenseObject(dafMedicationDispenseList.get(i)))))
					.append("\r\n");
		}
		try (FileWriter writer = new FileWriter(ndJsonFile, true); BufferedWriter bw = new BufferedWriter(writer)) {
			...
			log.error("error occured while writing to MedicationDispense file", e);
		}
	}
