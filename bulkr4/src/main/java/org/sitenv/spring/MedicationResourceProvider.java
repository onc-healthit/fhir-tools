...
public class MedicationResourceProvider implements IResourceProvider {
	Logger log = (Logger) LoggerFactory.getLogger(MedicationResourceProvider.class);
	 ...
	 * @param ctx
	 */
	public void getMedicationForBulkDataRequest(StringJoiner patients, Date start, File destDir, FhirContext ctx) {
		...
		for (int i = 0; i < dafMedicationList.size(); i++) {

			sb.append((ctx.newJsonParser().encodeResourceToString(createMedicationObject(dafMedicationList.get(i)))))
					.append("\r\n");
		}
		try (FileWriter writer = new FileWriter(ndJsonFile, true); BufferedWriter bw = new BufferedWriter(writer)) {
			...
			log.error("error occured while writing to Medication file", e);
		}
	}
