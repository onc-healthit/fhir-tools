...
public class MedicationStatementResourceProvider implements IResourceProvider {
	Logger log = (Logger) LoggerFactory.getLogger(MedicationStatementResourceProvider.class);
	...
	public void getMedicationStatementForBulkDataRequest(StringJoiner patients, Date start, File destDir, FhirContext ctx) {
		String fileName = "MedicationStatement.ndjson";
		List<DafMedicationStatement> dafMedicationStatementList = service.getMedicationStatementForBulkData(patients,
				start);
		File ndJsonFile = new File(destDir.getAbsolutePath() + "/" + fileName);
//		System.out.println("dafAllergy.." + dafMedicationStatementList.size());
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < dafMedicationStatementList.size(); i++) {

			sb.append((ctx.newJsonParser()
					.encodeResourceToString(createMedicationStatementObject(dafMedicationStatementList.get(i)))))
					.append("\r\n");
		}
		try (FileWriter writer = new FileWriter(ndJsonFile, true); BufferedWriter bw = new BufferedWriter(writer)) {
			...
			log.error("error occured while writing to MedicationStatement file", e);
		}
	}
