...
public class CarePlanDaoImpl extends AbstractDao implements CarePlanDao {
	...
	public List<DafCarePlan> getCarePlanForBulkData(StringJoiner patients, Date start) {
		List<DafCarePlan> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from careplan where data->").append("'").append("subject").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafCarePlan.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
