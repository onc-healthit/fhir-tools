...
public class CareTeamDaoImpl extends AbstractDao implements CareTeamDao {
	...
	public List<DafCareTeam> getCareTeamForBulkData(StringJoiner patients, Date start) {
		List<DafCareTeam> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from careteam where data->").append("'").append("subject").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafCareTeam.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
