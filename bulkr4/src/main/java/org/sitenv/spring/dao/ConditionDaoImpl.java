...
F	 */
	...
	public List<DafCondition> getConditionForBulkData(StringJoiner patients, Date start) {
		List<DafCondition> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from condition where data->").append("'").append("subject").append("'").append("->>")
	...
	//		System.out.println("query..." + query);
			list = getSession().createNativeQuery(query, DafCondition.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
