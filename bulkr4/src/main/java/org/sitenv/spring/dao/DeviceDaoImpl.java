...
public class DeviceDaoImpl extends AbstractDao implements DeviceDao {
	...
	public List<DafDevice> getDeviceForBulkData(StringJoiner patients, Date start) {
		List<DafDevice> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from device where data->").append("'").append("patient").append("'").append("->>")
	...
	//		System.out.println("query..." + query);
			list = getSession().createNativeQuery(query, DafDevice.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
