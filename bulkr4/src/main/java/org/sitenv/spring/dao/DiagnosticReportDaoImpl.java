...
public class DiagnosticReportDaoImpl extends AbstractDao implements DiagnosticReportDao {
	...
	public List<DafDiagnosticReport> getDiagnosticReportForBulkData(StringJoiner patients, Date start) {
		List<DafDiagnosticReport> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from report where data->").append("'").append("subject").append("'").append("->>")
			...
			list = getSession().createNativeQuery(query, DafDiagnosticReport.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
