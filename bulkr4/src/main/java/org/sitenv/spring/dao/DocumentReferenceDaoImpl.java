...
public class DocumentReferenceDaoImpl extends AbstractDao implements DocumentReferenceDao {
	...
	public List<DafDocumentReference> getDocumentReferenceForBulkData(StringJoiner patients, Date start) {
		List<DafDocumentReference> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from documentreference where data->").append("'").append("subject").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafDocumentReference.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
