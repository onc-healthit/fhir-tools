...
public class EncounterDaoImpl extends AbstractDao implements EncounterDao {
	...
	public List<DafEncounter> getEncounterForBulkData(StringJoiner patients, Date start) {
		List<DafEncounter> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from encounter where data->").append("'").append("subject").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafEncounter.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
