...
public class FamilyMemberHistoryDaoImpl extends AbstractDao implements FamilyMemberHistoryDao {
	...
	public List<DafFamilyMemberHistory> getFamilyMemberHistoryForBulkData(StringJoiner patients, Date start) {
		...
		List<DafFamilyMemberHistory> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from familymemberhistory where data->").append("'").append("patient").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafFamilyMemberHistory.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
