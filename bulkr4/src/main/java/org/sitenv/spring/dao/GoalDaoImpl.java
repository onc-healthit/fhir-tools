...
public class GoalDaoImpl extends AbstractDao implements GoalDao {
	...
	public List<DafGoal> getGoalForBulkData(StringJoiner patients, Date start) {
		List<DafGoal> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from goal where data->").append("'").append("subject").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafGoal.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
