...
public class HealthcareServiceDaoImpl extends AbstractDao implements HealthcareServiceDao{
	...
	public List<DafHealthcareService> getHealthcareServiceForBulkData(StringJoiner patients, Date start) {
		List<DafHealthcareService> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from healthcareservice where data->").append("'").append("patient").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafHealthcareService.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
