...
public class ImagingStudyDaoImpl extends AbstractDao implements ImagingStudyDao {
	...
	public List<DafImagingStudy> getImagingStudyForBulkData(StringJoiner patients, Date start) {
		List<DafImagingStudy> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from imagingstudy where data->").append("'").append("subject").append("'")
					.append("->>").append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafImagingStudy.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
