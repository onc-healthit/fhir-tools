...
public class ImmunizationDaoImpl extends AbstractDao implements ImmunizationDao {
	...
	public List<DafImmunization> getImmunizationForBulkData(StringJoiner patients, Date start) {
		List<DafImmunization> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from immunization where data->").append("'").append("patient").append("'")
					.append("->>").append("'").append("reference").append("'").append("in (");
			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafImmunization.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
