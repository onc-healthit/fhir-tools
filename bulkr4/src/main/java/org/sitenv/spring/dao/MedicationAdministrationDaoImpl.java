...
public class MedicationAdministrationDaoImpl extends AbstractDao implements MedicationAdministrationDao {
	...
	public List<DafMedicationAdministration> getMedicationAdministrationForBulkData(StringJoiner patients, Date start) {
		List<DafMedicationAdministration> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from allergy where data->").append("'").append("subject").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafMedicationAdministration.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
