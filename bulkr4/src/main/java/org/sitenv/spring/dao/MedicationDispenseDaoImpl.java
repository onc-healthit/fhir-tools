...
public class MedicationDispenseDaoImpl extends AbstractDao implements MedicationDispenseDao {
	...
	public List<DafMedicationDispense> getMedicationDispenseForBulkData(StringJoiner patients, Date start) {
		List<DafMedicationDispense> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from medicationdispense where data->").append("'").append("subject").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafMedicationDispense.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
