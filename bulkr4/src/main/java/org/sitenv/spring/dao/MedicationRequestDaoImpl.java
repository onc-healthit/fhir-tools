...
public class MedicationRequestDaoImpl extends AbstractDao implements MedicationRequestDao {
	 ...
	 */
	@Override
	public List<DafMedicationRequest> getMedicationRequestForBulkData(StringJoiner patients, Date start) {
		List<DafMedicationRequest> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from medicationrequest where data->").append("'").append("subject").append("'")
					.append("->>").append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafMedicationRequest.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
