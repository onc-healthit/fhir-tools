...
public class MedicationStatementDaoImpl extends AbstractDao implements MedicationStatementDao {
	...
	public List<DafMedicationStatement> getMedicationStatementForBulkData(StringJoiner patients, Date start) {
		List<DafMedicationStatement> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from medicationstatement where data->").append("'").append("subject").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafMedicationStatement.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
