...
public class ObservationDaoImpl extends AbstractDao implements ObservationDao {
	...
	public List<DafObservation> getObservationForBulkData(StringJoiner patients, Date start) {
		List<DafObservation> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from observation where data->").append("'").append("subject").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafObservation.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
