...
public class PatientDaoImpl extends AbstractDao implements PatientDao {
	Logger log = (Logger) LoggerFactory.getLogger(PatientDaoImpl.class);
	...
	/**
	 * This method builds criteria for fetching particular version of the patient
	 * record by id.
	 * 
	...
	public DafPatient getPatientByVersionId(String theId, String versionId) {
		DafPatient list = getSession().createNativeQuery("select * from patient where data->>'id' = '" + theId
				+ "' and data->'meta'->>'versionId' = '" + versionId + "'", DafPatient.class).getSingleResult();
		return list;
	}
					...
					if (givenName.isExact()) {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->'given'->>0 = '" + givenName.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->'given'->>1 = '" + givenName.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->'given'->>0 = '" + givenName.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->'given'->>1 = '" + givenName.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->'given'->>0 = '" + givenName.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->'given'->>1 = '" + givenName.getValue() + "'"));
					} else if (givenName.isContains()) {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->'given'->>0 ilike '%" + givenName.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->'given'->>1 ilike '%" + givenName.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->'given'->>0 ilike '%" + givenName.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->'given'->>1 ilike '%" + givenName.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->'given'->>0 ilike '%" + givenName.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->'given'->>1 ilike '%" + givenName.getValue() + "%'"));
					} else {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->'given'->>0 ilike '" + givenName.getValue() + "%'"),
			...
			for (List<? extends IQueryParameterType> values : list) {
				...
				for (IQueryParameterType params : values) {
					...
					if (familyName.isExact()) {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->>'family' = '" + familyName.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->>'family' = '" + familyName.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->>'family' = '" + familyName.getValue() + "'"));
					} else if (familyName.isContains()) {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->>'family' ilike '%" + familyName.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->>'family' ilike '%" + familyName.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->>'family' ilike '%" + familyName.getValue() + "%'"));
					} else {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->>'family' ilike '" + familyName.getValue() + "%'"),
			...
			for (List<? extends IQueryParameterType> values : list) {
				...
				for (IQueryParameterType params : values) {
					...
					if (telecom.isExact()) {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'telecom'->0->>'value' = '" + telecom.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'telecom'->1->>'value' = '" + telecom.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'telecom'->2->>'value' = '" + telecom.getValue() + "'"));
					} else if (telecom.isContains()) {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'telecom'->0->>'value' ilike '%" + telecom.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'telecom'->1->>'value' ilike '%" + telecom.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'telecom'->2->>'value' ilike '%" + telecom.getValue() + "%'"));
					} else {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'telecom'->0->>'value' ilike '" + telecom.getValue() + "%'"),
					...
					if (gender.getModifier() != null) {
						...
						if (modifier.getValue() == ":not") {
							criteria.add(Restrictions
									.sqlRestriction("{alias}.data->>'gender' not ilike '" + gender.getValue() + "'"));
						}
					} else if (StringUtils.isNoneEmpty(gender.getValue())) {
						criteria.add(Restrictions
								.sqlRestriction("{alias}.data->>'gender' ilike '%" + gender.getValue() + "%'"));
				...
				for (IQueryParameterType params : values) {
					...
					if (StringUtils.isNoneEmpty(deceased.getValue())) {
						criteria.add(Restrictions.sqlRestriction(
								"{alias}.data->>'deceasedBoolean' ilike '%" + deceased.getValue() + "%'"));
			...
			for (List<? extends IQueryParameterType> values : list) {
				...
				for (IQueryParameterType params : values) {
					...
					if (name.isExact()) {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->>'family' = '" + name.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->>'family' = '" + name.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->>'family' = '" + name.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->'given'->>0 = '" + name.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->'given'->>1 = '" + name.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->'given'->>0 = '" + name.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->'given'->>1 = '" + name.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->'given'->>0 = '" + name.getValue() + "'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->'given'->>1 = '" + name.getValue() + "'")

						);
					} else if (name.isContains()) {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->>'family' ilike '%" + name.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->>'family' ilike '%" + name.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->>'family' ilike '%" + name.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->'given'->>0 ilike '%" + name.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->'given'->>1 ilike '%" + name.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->'given'->>0 ilike '%" + name.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->1->'given'->>1 ilike '%" + name.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->'given'->>0 ilike '%" + name.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->2->'given'->>1 ilike '%" + name.getValue() + "%'"));
					} else {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'name'->0->>'family' ilike '" + name.getValue() + "%'"),
				...
				for (IQueryParameterType params : values) {
					...
					if (birthDate.getPrefix() != null) {
						if (birthDate.getPrefix().getValue() == "gt") {
							criteria.add(Restrictions
									.sqlRestriction("{alias}.data->>'birthDate' > '" + birthDateFormat + "'"));
			...
			for (List<? extends IQueryParameterType> values : list) {
				...
				for (IQueryParameterType params : values) {
					...
					if (addressPostalcode.isExact()) {
						...
						disjunction.add(Restrictions.sqlRestriction("{alias}.data->'address'->0->>'postalCode' ilike '%"
								+ addressPostalcode.getValue() + "%'"));
						disjunction.add(Restrictions.sqlRestriction("{alias}.data->'address'->1->>'postalCode' ilike '%"
								+ addressPostalcode.getValue() + "%'"));
		...
		if (list != null) {
			...
			for (List<? extends IQueryParameterType> values : list) {
				...
				for (IQueryParameterType params : values) {
					...
					if (language.isExact()) {
						disjunction.add(Restrictions.sqlRestriction(
								"{alias}.data->'communication'->0->'language'->'coding'->0->>'system' = '"
										+ language.getValue() + "'"));
						disjunction.add(Restrictions
								.sqlRestriction("{alias}.data->'communication'->0->'language'->'coding'->0->>'code' = '"
										+ language.getValue() + "'"));
						disjunction.add(Restrictions.sqlRestriction(
								"{alias}.data->'communication'->0->'language'->'coding'->0->>'display' = '"
										+ language.getValue() + "'"));

					} else if (language.isContains()) {
						disjunction.add(Restrictions.sqlRestriction(
								"{alias}.data->'communication'->0->'language'->'coding'->0->>'system' ilike '%"
										+ language.getValue() + "%'"));
						disjunction.add(Restrictions.sqlRestriction(
								"{alias}.data->'communication'->0->'language'->'coding'->0->>'code' ilike '%"
										+ language.getValue() + "%'"));
						disjunction.add(Restrictions.sqlRestriction(
								"{alias}.data->'communication'->0->'language'->'coding'->0->>'display' ilike '%"
										+ language.getValue() + "%'"));
			...
			for (List<? extends IQueryParameterType> values : list) {
				for (IQueryParameterType params : values) {
					...
					if (link.getValue() != null) {
						criteria.add(Restrictions.sqlRestriction(
								"{alias}.data->'link'->0->'other'->>'reference' = '" + link.getValue() + "'"));
					} else if (link.getMissing()) {
						criteria.add(
								Restrictions.sqlRestriction("{alias}.data->'link'->0->'other'->>'reference' IS NULL"));
					} else if (!link.getMissing()) {
						criteria.add(Restrictions
								.sqlRestriction("{alias}.data->'link'->0->'other'->>'reference' IS NOT NULL"));
					}
				}
			}
		}
	...
	/**
	 * This method builds criteria for organization that is the custodian of the
	 * patient record
	 * 
			...
			for (List<? extends IQueryParameterType> values : list) {
				...
				for (IQueryParameterType params : values) {
					...
					if (organization.isExact()) {
						disjunction
								.add(Restrictions.sqlRestriction("{alias}.data->'managingOrganization'->>'display' = '"
						...
						disjunction.add(
								Restrictions.sqlRestriction("{alias}.data->'managingOrganization'->>'display' ilike '%"
										+ organization.getValue() + "%'"));
						disjunction.add(Restrictions
								.sqlRestriction("{alias}.data->'managingOrganization'->>'reference' ilike '%"
										+ organization.getValue() + "%'"));
						...
						disjunction.add(
								Restrictions.sqlRestriction("{alias}.data->'managingOrganization'->>'display' ilike '"
										+ organization.getValue() + "%'"));
						disjunction.add(
								Restrictions.sqlRestriction("{alias}.data->'managingOrganization'->>'reference' ilike '"
										+ organization.getValue() + "%'"));
			...
			for (List<? extends IQueryParameterType> values : list) {
				...
				for (IQueryParameterType params : values) {
					...
					if (addressUse.getValue() != null) {
						orCond = Restrictions.or(
								Restrictions.sqlRestriction(
										"{alias}.data->'address'->0->>'use' ilike '%" + addressUse.getValue() + "%'"),
								Restrictions.sqlRestriction(
										"{alias}.data->'address'->1->>'use' ilike '%" + addressUse.getValue() + "%'"));
					}
					disjunction.add(orCond);
				}
				criteria.add(disjunction);
	...
	public List<DafPatient> getPatientHistoryById(String theId) {
		List<DafPatient> list = getSession().createNativeQuery(
				"select * from patient where data->>'id' = '" + theId + "' order by data->'meta'->>'versionId' desc",
				DafPatient.class).getResultList();
		return list;
	}
	...
	public List<DafPatient> getPatientsOnAuthorize() {
		 ...
		 */
		// List<DafPatient> list = getSession().createNativeQuery(
		// "SELECT * FROM patient WHERE (data->>'id', data->'meta'->>'versionId') IN (
		// SELECT data->>'id', MAX(data->'meta'->>'versionId') FROM patient GROUP BY
		// data->>'id' ) ORDER BY data->>'id'" , DafPatient.class) .getResultList();

		List<DafPatient> list = getSession()
				.createNativeQuery("SELECT * FROM patient  ORDER BY data->>'id'", DafPatient.class).getResultList();
		return list;
	}

//	@Override
//    @Transactional
//    public List<DafPatient> getAllPatientJsonForBulkData(List<String> patients, Date start){
//    	
//    	Criteria criteria = getSession().createCriteria(DafPatient.class);
//               
//    			if(patients!=null) {
...
//	}
	...
	/**
	 * fetch the results for bulk data
	 */
	@Override
	@Transactional
	public List<DafPatient> getAllPatientJsonForBulkData(StringJoiner patients, Date start) {
		List<DafPatient> list = null;
		List<String> arrayToList = new ArrayList<String>();
		try {
			
			String str[] = patients.toString().replaceAll("Patient/", "").split(",");
			arrayToList = Arrays.asList(str);
			
			StringBuffer stb = new StringBuffer();
			stb.append("select * from patient where data->>").append("'").append("id").append("'").append("in (");

			arrayToList.forEach(x -> {
				stb.append(x).append(",");
			});
			...
			list = getSession().createNativeQuery(query, DafPatient.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
