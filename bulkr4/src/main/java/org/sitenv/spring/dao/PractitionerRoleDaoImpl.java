...
*/
	...
	public List<DafPractitionerRole> getPractitionerRoleForBulkData(StringJoiner patients, Date start) {
			List<DafPractitionerRole> list = null;
			try {
				StringBuffer stb = new StringBuffer();
				stb.append("select * from practitioner_role where data->").append("'").append("subject").append("'").append("->>")
						.append("'").append("reference").append("'").append("in (");

				stb.append(patients);
				stb.append(")");
				String query = stb.toString();
				list = getSession().createNativeQuery(query, DafPractitionerRole.class).getResultList();
			} catch (Exception e) {
				e.printStackTrace();
			}
			return list;
