...
public class ProcedureDaoImpl extends AbstractDao implements ProcedureDao {
	...
	public List<DafProcedure> getProcedureForBulkData(StringJoiner patients, Date start) {
		List<DafProcedure> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from procedure where data->").append("'").append("subject").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafProcedure.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
