...
public class RiskAssessmentDaoImpl  extends AbstractDao implements RiskAssessmentDao {
	...
	public List<DafRiskAssessment> getRiskAssessmentForBulkData(StringJoiner patients, Date start) {
		List<DafRiskAssessment> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from riskassessment where data->").append("'").append("subject").append("'")
					.append("->>").append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafRiskAssessment.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
