...
public class SpecimenDaoImpl extends AbstractDao implements SpecimenDao {
	...
	public List<DafSpecimen> getSpecimenRequestDataRequest(StringJoiner patients, Date start) {
		List<DafSpecimen> list = null;
		try {
			StringBuffer stb = new StringBuffer();
			stb.append("select * from specimen where data->").append("'").append("subject").append("'").append("->>")
					.append("'").append("reference").append("'").append("in (");

			stb.append(patients);
			stb.append(")");
			String query = stb.toString();
			list = getSession().createNativeQuery(query, DafSpecimen.class).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
