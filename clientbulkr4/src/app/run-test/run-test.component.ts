...
export class RunTestComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.checkBulkData();
    this.secureFHIRServer = 'https://fhir.sitenv.org/secure-fhir/fhir';
    this.fhirserverurl = 'https://fhir.sitenv.org/backend-app';
    this.exportAllContentLocation =
      'https://fhir.sitenv.org/backend-app/bulkdata/444';
    this.extractByGroupContentLocation =
      'https://fhir.sitenv.org/backend-app/bulkdata/445';
    this.exportUsingParamsContentLocation =
      'https://fhir.sitenv.org/backend-app/bulkdata/446';
    this.exportUsingGroupParamsContentLocation =
      'https://fhir.sitenv.org/backend-app/bulkdata/447';
    this.resourcesList = [
      'AllergyIntolerance',
      'CarePlan',
      'Claim',
  ...
  exportAll() {
    ...
    this.runTestService
      ...
      .subscribe((res: Response) => {
        ...
        this.extractAllUrl =
          'https://fhir.sitenv.org/secure-fhir/fhir/Patient/$export';
        const result = this.responseBody.output;
        this.extractAllData = [];
        for (let p = 0; p < result.length; p++) {
          const dataObj = {
    ...
    this.runTestService
      ...
      .subscribe((res: Response) => {
        ...
        this.exportByGroupIdUrl =
          'https://fhir.sitenv.org/secure-fhir/fhir/Group/5/$export';
        const result = this.responseBody.output;
        this.extractData = [];
        for (let p = 0; p < result.length; p++) {
          const dataObj = {
    ...
    this.runTestService
      ...
      .subscribe((res: Response) => {
        ...
        this.exportUsingParamsUrl =
          'https://fhir.sitenv.org/secure-fhir/fhir/Patient/$export?_since=1990-01-01&_type=AllergyIntolerance,Condition';
        this.extractParamsData = [];
        for (let p = 0; p < result.length; p++) {
          const dataObj = {
            resourceName: this.util.capitalizeFirstChar(result[p].type),
    ...
    this.runTestService
      ...
      .subscribe((res: Response) => {
        ...
        this.exportGroupUsingParamsUrl =
          'https://fhir.sitenv.org/secure/fhir/Group/5/$export?_since=1990-01-01&_type=AllergyIntolerance,Condition';
        this.extractGroupParamsData = [];
        for (let p = 0; p < result.length; p++) {
          const dataObj = {
            resourceName: this.util.capitalizeFirstChar(result[p].type),
