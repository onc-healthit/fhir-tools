(function(){
    ...
    loadfhirres = function(){
        ...
        var label= $('#confprofname').val();

        $('#fhirresources').children().remove();
        $('#fhirresources').append('<option>Select Profile</option>');
        populateSelectIdentifiedById('fhirresources',label);
    ...
    getdemouser=function(val) {
        if(val == "dstu2"){
            var newURL = window.location.protocol + "//" + window.location.host + "/open/client/democlient";
            var demoURL = window.location.protocol + "//" + window.location.host + "/secure/fhir"
        }else{
            var newURL = window.location.protocol + "//" + window.location.host + "/open/stu3/client/democlient";
            var demoURL = window.location.protocol + "//" + window.location.host + "/secure/stu3/fhir"
        }
    ...
    populateSelectIdentifiedById = function( selId ,label) {
        console.log(label+"-----"+JSON.stringify(fhirresources[label]));
        populateSelectControlWithAssociativeArray( $('#'+selId)[0], fhirresources[label] );
    };
    ...
    runresourcetest =  function(){
        ...
        l.ladda( 'start' );
        getoauthjson(strurl,searchparamarr,searchparamval,l,confprofname);
    }

    getoauthjson = function(strurl,searchparamarr,searchparamval,l,selectedstu){
        ...
        if(localStorage.getItem("authtype") == 'auth'){
            $.ajax({
                ...
                success:function(data,status,xhr){
                    ...
                    if(selectedstu == "DSTU2"){
                        dstu2tableview(resourcename,data,tab);
                    }else if(selectedstu == "STU3"){
                        stu3tableview(resourcename,data,tab);
                    }
                ...
                error:function(e){
                    ...
                    if(e.status == '401'){
                        ...
                        var authfail = $('<div class="alert alert-danger" id="serverauthorized" style="text-align:center;margin-bottom:0px;padding:6px 12px;"><strong>'+$(foundElement[0]).text()+'</strong></div>');
            ...
            $.ajax({
                ...
                error:function(e){
                    ...
                    if(e.status == '401'){
                        ...
                        var parseHtml = $(e.responseText);
                        var foundElement = parseHtml.find('u');
                        var authfail = $('<div class="alert alert-danger" id="serverauthorized" style="text-align:center;margin-bottom:0px;padding:6px 12px;"><strong>'+$(foundElement[1]).text()+'</strong></div>');
                        $('#serverResponse').append(authfail);
                    }else{
                        $('#serverResponse').html('');
                        if(e.responseJSON != undefined){
                            ...
                            var authfail = $('<div class="alert alert-danger" id="serverauthorized" style="text-align:center;margin-bottom:0px;padding:6px 12px;"><strong>The requested resource is not available.</strong></div>');
                        }
                        $('#serverResponse').append(authfail);
                    }
                    //window.scrollTo(0, 0);
