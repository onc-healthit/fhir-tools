(function(){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
		        ...
		        $('#authsuccess').append(authsuccess);
	          	renderdeviceresults(data,strurl,xhr,deviceByPatientId,deviceCollapseByPatientId,resType);
	          	l.ladda('stop');
	        },
	        error:function(e){
	        	devicetrigger = 1;
	        	...
	        	renderdeviceerror(e,deviceByPatientId);
	   		}
		})
	}

	renderdeviceresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="deviceremovabletr"></tr>');
	...
	renderdeviceerror = function(e,trid){
		var authfail;
		tr = $('<tr class="deviceremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
