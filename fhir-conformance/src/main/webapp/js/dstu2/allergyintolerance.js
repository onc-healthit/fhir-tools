(function(){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
		        ...
		        $('#authsuccess').append(authsuccess);
	          	renderallergyresults(data,strurl,xhr,allergyByPatId,colallergyByPatId,resType);
	          	l.ladda('stop');
	        },
	        error:function(e){
	        	allergytrigger =1;
	        	...
	        	renderallergyerror(e,allergyByPatId);
	   		}
		})
	}

	renderallergyresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="allergyremovabletr"></tr>');
	...
	renderallergyerror = function(e,trid){
		var authfail;
		tr = $('<tr class="allergyremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
