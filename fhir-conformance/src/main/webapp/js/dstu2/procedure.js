(function(){
	...
	var procedureByPatientId = "procedureByPatientId";
	var procedureCollapseByPatientId = "procedureCollapseByPatientId";
	var procedureByPatientIdandDate = "procedureByPatientIdandDate";
	var procedureCollapseByPatientIdandDate = "procedureCollapseByPatientIdandDate";
	var l = $( '#executebtn' ).ladda();
	procedureByPatId = function(strurl){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	$('.procedurerunByPatId').html('Passed');
	          	if(proceduretrigger == 0){
	          		$('.procedurerun').html('Passed');
	          	}
	          	$('.procedurepass').html(proceduresuccesscount);
	          	$('#authsuccess').html('');
		        ...
		        procedurebyPatIdandDate(strurl,data,procedureByPatientIdandDate,procedureCollapseByPatientIdandDate);
	          	renderprocedureresults(data,strurl,xhr,procedureByPatientId,procedureCollapseByPatientId,resType);
	          	l.ladda('stop');
	        },
	        error:function(e){
	        	proceduretrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.procedurerunByPatId').html('Failed');
	        	$('.procedurerun').html('Failed');
	        	$('.procedurerun').parent().addClass('bg-danger');
	        	renderprocedureerror(e,procedureByPatientId);
	   		}
		})
	}

	procedurebyPatIdandDate = function(strurl,data,procedureByPatientIdandDate,procedureCollapseByPatientIdandDate){
		//console.log(data);
		var datearr=[];
			if(data.entry){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	proceduresuccesscount++;
	          	$('.procedurerunByPatIdandDate').html('Passed');
	          	if(proceduretrigger == 0){
	          		$('.procedurerun').html('Passed');
	          	}
	          	$('.procedurepass').html(proceduresuccesscount);
	          	renderprocedureresults(data,strurl,xhr,procedureByPatientIdandDate,procedureCollapseByPatientIdandDate,resType);
	        },
	        error:function(e){
	        	proceduretrigger =1;
	        	l.ladda( 'stop' );
	        	$('.procedurerunByPatIdandDate').html('Failed');
	        	$('.procedurerun').html('Failed');
	        	$('.procedurerun').parent().addClass('bg-danger');
	        	renderprocedureerror(e,procedureByPatientIdandDate);
	   		}
		});
	}

	renderprocedureresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="procedureremovabletr"></tr>');
	...
	renderprocedureerror = function(e,trid){
		var authfail;
		tr = $('<tr class="procedureremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
