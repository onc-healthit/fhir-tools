(function(){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
		        ...
		        $('#authsuccess').append(authsuccess);
	          	renderimmunizationresults(data,strurl,xhr,immunizationByPatientId,immunizationCollapseByPatientId,resType);
	          	l.ladda('stop');
	        },
	        error:function(e){
	        	...
	        	renderimmunizationerror(e,immunizationByPatientId);
	   		}
		})
	}

	renderimmunizationresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="immunizationremovabletr"></tr>');
	...
	renderimmunizationerror = function(e,trid){
		var authfail;
		tr = $('<tr class="immunizationremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
