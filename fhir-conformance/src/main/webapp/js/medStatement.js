(function(){
	...
	var medStatementByPatientIdandinclude = "medStatementByPatientIdandinclude";
	var medStatementCollapseByPatientIdandinclude = "medStatementCollapseByPatientIdandinclude";
	...
	medicationStatementByPatientId = function(strurl){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
		        ...
		        medicationStatementByPatientIdandInclude(strurl,data,medStatementByPatientIdandinclude,medStatementCollapseByPatientIdandinclude);
	          	rendermedstatementresults(data,strurl,xhr,medStatementByPatientId,medStatementCollapseByPatientId,resType);
	          	l.ladda('stop');
	        },
	        error:function(e){
	        	...
	        	rendermedstatementerror(e,medStatementByPatientId);
	   		}
		})
	}

	medicationStatementByPatientIdandInclude = function(strurl,data,medStatementByPatientIdandinclude,medStatementCollapseByPatientIdandinclude){
		var patientid = localStorage.getItem("patientid");
		l.ladda('start');
		access_token = localStorage.getItem("access_token");
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	medStatementsuccesscount++;
	          	$('.medStatementrunByPatIdandinclude').html('Passed');
	          	if(medstatementtrigger == 0){
	          		$('.medStatementrun').html('Passed');
	          	}
	          	$('.medStatementpass').html(medStatementsuccesscount);
	          	rendermedstatementresults(data,strurl,xhr,medStatementByPatientIdandinclude,medStatementCollapseByPatientIdandinclude,resType);
	        },
	        error:function(e){
	        	medstatementtrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.medStatementrunByPatIdandinclude').html('Failed');
	        	$('.medStatementrun').html('Failed');
	        	$('.medStatementrun').parent().addClass('bg-danger');
	        	rendermedstatementerror(e,medStatementByPatientIdandinclude);
	   		}
		})
	}

	rendermedstatementresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="medStatementremovabletr"></tr>');
	...
	rendermedstatementerror = function(e,trid){
		var authfail;
		tr = $('<tr class="medStatementremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
