(function(){
	...
	var stu3allergyByPatId = "stu3allergyByPatId";
	var stu3colallergyByPatId = "stu3byPatId";
	var l = $( '#executebtn' ).ladda();
	stu3allergyByPatientId = function(strurl){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	allergysuccesscount++;
	          	$('.stu3allergyrunByPatId').html('Passed');
	          	if(allergytrigger == 0){
	          		$('.stu3allergyrun').html('Passed');
	          	}
	          	$('.stu3allergypass').html(allergysuccesscount);
	          	$('#authsuccess').html('');
		        ...
		        $('#authsuccess').append(authsuccess);
	          	stu3renderallergyresults(data,strurl,xhr,stu3allergyByPatId,stu3colallergyByPatId,resType);
	          	l.ladda('stop');
	        },
	        error:function(e){
	        	allergytrigger =1;
	        	l.ladda( 'stop' );
	        	$('.stu3allergyrunByPatId').html('Failed');
	        	$('.stu3allergyrun').html('Failed');
	        	$('.stu3allergyrun').parent().addClass('bg-danger');
	        	stu3renderallergyerror(e,stu3allergyByPatId);
	   		}
		})
	}

	stu3renderallergyresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="stu3allergyremovabletr"></tr>');
	...
	stu3renderallergyerror = function(e,trid){
		var authfail;
		tr = $('<tr class="stu3allergyremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
