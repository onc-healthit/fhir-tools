(function(){
	...
	var ctSuccessCount = 0;
	var cttrigger = 0;
	var stu3ctByPatientId = "stu3ctByPatientId";
	var stu3ctCollapseByPatientId = "stu3ctCollapseByPatientId";
	var stu3ctByPatStatus = "stu3ctByPatStatus";
	var stu3ctCollapseByPatientStatus = "stu3ctCollapseByPatientStatus";
	
	var l = $( '#executebtn' ).ladda();
	stu3ctByPatient =  function(strurl){
		var patientid = localStorage.getItem("patientid");
		l.ladda('start');
		ctSuccessCount = 0;
		access_token = localStorage.getItem("access_token");
		strurl = strurl+ "/CareTeam?patient="+patientid + "&_format=json";
		$.ajax({
      		url:strurl,
        	type:"GET",
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	ctSuccessCount++;
	          	$('.stu3ctrunByPatId').html('Passed');
	          	if(cttrigger == 0){
	          		$('.stu3ctrun').html('Passed');
	          	}
	          	$('.stu3ctpass').html(ctSuccessCount);
	          	$('#authsuccess').html('');
		        ...
		        $('#authsuccess').append(authsuccess);
	          	stu3ctByPatientStatus(strurl,data,stu3ctByPatStatus,stu3ctCollapseByPatientStatus);
	          	stu3renderctresults(data,strurl,xhr,stu3ctByPatientId,stu3ctCollapseByPatientId,resType);
	          	l.ladda( 'stop' );
	        },
	        error:function(e){
	        	cttrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.stu3ctrunByPatId').html('Failed');
	        	$('.stu3ctrun').html('Failed');
	        	$('.stu3ctpass').parent().addClass('bg-danger');
	        	stu3rendercterror(e,stu3ctByPatientId);
	   		}
		})
	}

	stu3ctByPatientStatus = function(strurl,data,stu3ctByPatStatus,stu3ctCollapseByPatientStatus){
		var category,status;
		if(data.entry){
		...
		strurl = strurl + "/CareTeam?patient="+patientid+"&status="+status+"&_format=json";
		access_token = localStorage.getItem("access_token");
		$.ajax({
      		url:strurl,
        	type:"GET",
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	ctSuccessCount++;
	          	$('.stu3ctrunByPatStatus').html('Passed');
	          	if(cttrigger == 0){
	          		$('.stu3ctrun').html('Passed');
	          	}
	          	$('.stu3ctpass').html(ctSuccessCount);
	          	stu3renderctresults(data,strurl,xhr,stu3ctByPatStatus,stu3ctCollapseByPatientStatus,resType);
	        },
	        error:function(e){
	        	cttrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.stu3ctrunByPatCategoryStatus').html('Failed');
	        	$('.stu3ctrun').html('Failed');
	        	$('.stu3ctpass').parent().addClass('bg-danger');
	        	stu3rendercterror(e,stu3ctByPatCategoryStatus);
	   		}
		});
	}

	stu3renderctresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="stu3ctremovabletr"></tr>');
	...
	stu3rendercterror = function(e,trid){
		var authfail;
		tr = $('<tr class="stu3ctremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
