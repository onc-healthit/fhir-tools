(function(){
	...
	var devicetrigger = 0;
	var stu3deviceByPatientId = "stu3deviceByPatientId";
	var stu3deviceCollapseByPatientId = "stu3deviceCollapseByPatientId";
	var l = $( '#executebtn' ).ladda();
	stu3deviceByPatId = function(strurl){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	devicesuccesscount++;
	          	$('.stu3devicerunByPatId').html('Passed');
	          	if(devicetrigger == 0){
	          		$('.stu3devicerun').html('Passed');
	          	}
	          	$('.stu3devicepass').html(devicesuccesscount);
	          	$('#authsuccess').html('');
		        ...
		        $('#authsuccess').append(authsuccess);
	          	stu3renderdeviceresults(data,strurl,xhr,stu3deviceByPatientId,stu3deviceCollapseByPatientId,resType);
	          	l.ladda('stop');
	        },
	        error:function(e){
	        	devicetrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.stu3devicerunByPatId').html('Failed');
	        	$('.stu3devicerun').html('Failed');
	        	$('.stu3devicerun').parent().addClass('bg-danger');
	        	stu3renderdeviceerror(e,stu3deviceByPatientId);
	   		}
		})
	}

	stu3renderdeviceresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="stu3deviceremovabletr"></tr>');
	...
	stu3renderdeviceerror = function(e,trid){
		var authfail;
		tr = $('<tr class="stu3deviceremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
