(function(){
	...
	var docrefsuccesscount = 0;
	var docreftrigger = 0;
	var stu3docrefByPatientId = "stu3docrefByPatientId";
	var stu3docrefCollapseByPatientId = "stu3docrefCollapseByPatientId";
	var stu3docrefByPatientIdType = "stu3docrefByPatientIdType";
	var stu3docrefCollapseByPatientIdType = "stu3docrefCollapseByPatientIdType";
	var stu3docrefByPatientIdTypePeriod = "stu3docrefByPatientIdTypePeriod";
	var stu3docrefCollapseByPatientIdTypePeriod = "stu3docrefCollapseByPatientIdTypePeriod";
	var stu3docrefByextandPatientId = "stu3docrefByextandPatientId";
	var stu3docrefCollapseByextandPatientId = "stu3docrefCollapseByextandPatientId";
	var l = $( '#executebtn' ).ladda();
	stu3docrefByPatId = function(strurl){
		var patientid = localStorage.getItem("patientid");
		l.ladda('start');
		docrefsuccesscount = 0;
		access_token = localStorage.getItem("access_token");
		strurl = strurl+ "/DocumentReference?patient="+patientid + "&_format=json";
		$.ajax({
      		url:strurl,
        	type:"GET",
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	docrefsuccesscount++;
	          	$('.stu3docrefrunByPatId').html('Passed');
	          	if(docreftrigger == 0){
	          		$('.stu3docrefrun').html('Passed');
	          	}
	          	$('.stu3docrefpass').html(docrefsuccesscount);
	          	$('#authsuccess').html('');
		        ...
		        stu3docrefByPatientIdandType(strurl,data,stu3docrefByPatientIdType,stu3docrefCollapseByPatientIdType);
		        stu3docrefByPatientIdTypeandPeriod(strurl,data,stu3docrefByPatientIdTypePeriod,stu3docrefCollapseByPatientIdTypePeriod);
		        stu3docrefByextendedoperation(strurl,data,stu3docrefByextandPatientId,stu3docrefCollapseByextandPatientId);
	          	stu3renderdocrefresults(data,strurl,xhr,stu3docrefByPatientId,stu3docrefCollapseByPatientId,resType);
	          	l.ladda('stop');
	        },
	        error:function(e){
	        	docreftrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.stu3docrefrunByPatId').html('Failed');
	        	$('.stu3docrefrun').html('Failed');
	        	$('.stu3docrefrun').parent().addClass('bg-danger');
	        	stu3renderdocreferror(e,stu3docrefByPatientId);
	   		}
		})
	}

	stu3docrefByPatientIdandType = function(strurl,data,stu3docrefByPatientIdType,stu3docrefCollapseByPatientIdType){
		var typesystem,typevalue;
		if(data.entry){
	        typesystem =  data.entry[0].resource.type.coding[0].system;
	        typevalue = data.entry[0].resource.type.coding[0].code;
	    }else{
	    	if(data.type.coding){
	    		typesystem =  data.type.coding[0].system;
	        	typevalue = data.type.coding[0].code;
	    	}else{
	    		return;
	    	}
	    }
		...
		strurl = strurl + "/DocumentReference?patient="+patientid+"&type="+typesystem+"|"+typevalue+"&_format=json";
		access_token = localStorage.getItem("access_token");
		$.ajax({
      		url:strurl,
        	type:"GET",
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	docrefsuccesscount++;
	          	$('.stu3docrefrunByPatIdType').html('Passed');
	          	if(docreftrigger == 0){
	          		$('.stu3docrefrun').html('Passed');
	          	}
	          	$('.stu3docrefpass').html(docrefsuccesscount);
	          	stu3renderdocrefresults(data,strurl,xhr,stu3docrefByPatientIdType,stu3docrefCollapseByPatientIdType,resType);
	        },
	        error:function(e){
	        	docreftrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.stu3docrefrunByPatIdType').html('Failed');
	        	$('.stu3docrefrun').html('Failed');
	        	$('.stu3docrefpass').parent().addClass('bg-danger');
	        	stu3renderdocreferror(e,stu3docrefByPatientIdType);
	   		}
		});
	}

	stu3docrefByPatientIdTypeandPeriod = function(strurl,data,stu3docrefByPatientIdTypePeriod,stu3docrefCollapseByPatientIdTypePeriod){
		var typesystem,typevalue,period;
		if(data.entry){
	        typesystem =  data.entry[0].resource.type.coding[0].system;
	        typevalue = data.entry[0].resource.type.coding[0].code;
	        period = data.entry[0].resource.context.period.start;
	    }else{
	    	if(data.type.coding){
	    		typesystem =  data.type.coding[0].system;
	        	typevalue = data.type.coding[0].code;
	    	...
	    	if(data.context){
	    		period = data.context.period.start;
	    	}else{ return; }
	    }
		var patientid = localStorage.getItem("patientid");
		var strurl = localStorage.getItem("strurl");
		strurl = strurl + "/DocumentReference?patient="+patientid+"&type="+typesystem+"|"+typevalue+"&period="+period+"&_format=json";
		access_token = localStorage.getItem("access_token");
		$.ajax({
      		url:strurl,
        	type:"GET",
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	docrefsuccesscount++;
	          	$('.stu3docrefrunByPatIdTypePeriod').html('Passed');
	          	if(docreftrigger == 0){
	          		$('.stu3docrefrun').html('Passed');
	          	}
	          	$('.stu3docrefpass').html(docrefsuccesscount);
	          	stu3renderdocrefresults(data,strurl,xhr,stu3docrefByPatientIdTypePeriod,stu3docrefCollapseByPatientIdTypePeriod,resType);
	        },
	        error:function(e){
	        	docreftrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.stu3docrefrunByPatIdTypePeriod').html('Failed');
	        	$('.stu3docrefrun').html('Failed');
	        	$('.stu3docrefpass').parent().addClass('bg-danger');
	        	stu3renderdocreferror(e,stu3docrefByPatientIdTypePeriod);
	   		}
		});
	}

	stu3docrefByextendedoperation = function(strurl,data,stu3docrefByextandPatientId,stu3docrefCollapseByextandPatientId){
		var patientid = localStorage.getItem("patientid");
		var strurl = localStorage.getItem("strurl");
		strurl = strurl + "/DocumentReference/$docref?patient="+patientid+"&_format=json";
		access_token = localStorage.getItem("access_token");
		$.ajax({
      		url:strurl,
        	type:"GET",
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	docrefsuccesscount++;
	          	$('.stu3docrefrunByPatIdext').html('Passed');
	          	if(docreftrigger == 0){
	          		$('.stu3docrefrun').html('Passed');
	          	}
	          	$('.stu3docrefpass').html(docrefsuccesscount);
	          	stu3renderdocrefresults(data,strurl,xhr,stu3docrefByextandPatientId,stu3docrefCollapseByextandPatientId,resType);
	        },
	        error:function(e){
	        	docreftrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.stu3docrefrunByPatIdext').html('Failed');
	        	$('.stu3docrefrun').html('Failed');
	        	$('.stu3docrefpass').parent().addClass('bg-danger');
	        	stu3renderdocreferror(e,stu3docrefByextandPatientId);
	   		}
		});
	}

	stu3renderdocrefresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="stu3docrefremovabletr"></tr>');
	...
	stu3renderdocreferror = function(e,trid){
		var authfail;
		tr = $('<tr class="stu3docrefremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
