(function(){
	...
	var goaltrigger = 0;
	var stu3goalByPatientId = "stu3goalByPatientId";
	var stu3goalCollapseByPatientId = "stu3goalCollapseByPatientId";
	var stu3goalByPatientIdandDate = "stu3goalByPatientIdandDate";
	var stu3goalCollapseByPatientIdandDate = "stu3goalCollapseByPatientIdandDate";
	var l = $( '#executebtn' ).ladda();
	stu3goalByPatId = function(strurl){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	goalsuccesscount++;
	          	$('.stu3goalrunByPatId').html('Passed');
	          	if(goaltrigger == 0){
	          		$('.stu3goalrun').html('Passed');
	          	}
	          	$('.stu3goalpass').html(goalsuccesscount);
	          	$('#authsuccess').html('');
		        ...
		        stu3goalbyPatIdandDate(strurl,data,stu3goalByPatientIdandDate,stu3goalCollapseByPatientIdandDate);
	          	stu3rendergoalresults(data,strurl,xhr,stu3goalByPatientId,stu3goalCollapseByPatientId,resType);
	          	l.ladda('stop');
	        },
	        error:function(e){
	        	goaltrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.stu3goalrunByPatId').html('Failed');
	        	$('.stu3goalrun').html('Failed');
	        	$('.stu3goalrun').parent().addClass('bg-danger');
	        	stu3rendergoalerror(e,stu3goalByPatientId);
	   		}
		})
	}

	stu3goalbyPatIdandDate = function(strurl,data,stu3goalByPatientIdandDate,stu3goalCollapseByPatientIdandDate){
		//console.log(data);
		var datearr=[];
			if(data.entry){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	goalsuccesscount++;
	          	$('.stu3goalrunByPatIdandDate').html('Passed');
	          	if(goaltrigger == 0){
	          		$('.stu3goalrun').html('Passed');
	          	}
	          	$('.stu3goalpass').html(goalsuccesscount);
	          	stu3rendergoalresults(data,strurl,xhr,stu3goalByPatientIdandDate,stu3goalCollapseByPatientIdandDate,resType);
	        },
	        error:function(e){
	        	goaltrigger =1;
	        	l.ladda( 'stop' );
	        	$('.stu3goalrunByPatIdandDate').html('Failed');
	        	$('.stu3goalrun').html('Failed');
	        	$('.stu3goalrun').parent().addClass('bg-danger');
	        	stu3rendergoalerror(e,stu3goalByPatientIdandDate);
	   		}
		});
	}

	stu3rendergoalresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="stu3goalremovabletr"></tr>');
	...
	stu3rendergoalerror = function(e,trid){
		var authfail;
		tr = $('<tr class="stu3goalremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
