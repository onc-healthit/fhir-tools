(function(){
	...
	var stu3immunizationByPatientId = "stu3immunizationByPatientId";
	var stu3immunizationCollapseByPatientId = "stu3immunizationCollapseByPatientId";
	var l = $( '#executebtn' ).ladda();
	stu3immunizationByPatId = function(strurl){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	$('.stu3immunizationrunByPatId').html('Passed');
	          	if(immunizationtrigger == 0){
	          		$('.stu3immunizationrun').html('Passed');
	          	}
	          	$('.stu3immunizationpass').html(immunizationsuccesscount);
	          	$('#authsuccess').html('');
		        ...
		        $('#authsuccess').append(authsuccess);
	          	stu3renderimmunizationresults(data,strurl,xhr,stu3immunizationByPatientId,stu3immunizationCollapseByPatientId,resType);
	          	l.ladda('stop');
	        },
	        error:function(e){
	        	immunizationtrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.stu3immunizationrunByPatId').html('Failed');
	        	$('.stu3immunizationrun').html('Failed');
	        	$('.stu3immunizationrun').parent().addClass('bg-danger');
	        	stu3renderimmunizationerror(e,stu3immunizationByPatientId);
	   		}
		})
	}

	stu3renderimmunizationresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="stu3immunizationremovabletr"></tr>');
	...
	stu3renderimmunizationerror = function(e,trid){
		var authfail;
		tr = $('<tr class="stu3immunizationremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
