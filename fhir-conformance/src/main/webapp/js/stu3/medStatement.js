(function(){
	...
	var stu3medStatementByPatientId = "stu3medStatementByPatientId";
	var stu3medStatementCollapseByPatientId = "stu3medStatementCollapseByPatientId";
	var stu3medStatementByPatientIdandinclude = "stu3medStatementByPatientIdandinclude";
	var stu3medStatementCollapseByPatientIdandinclude = "stu3medStatementCollapseByPatientIdandinclude";
	var l = $( '#executebtn' ).ladda();
	stu3medicationStatementByPatientId = function(strurl){
		...
		$.ajax({
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	$('.stu3medStatementrunByPatId').html('Passed');
	          	if(medstatementtrigger == 0){
	          		$('.stu3medStatementrun').html('Passed');
	          	}
	          	$('.stu3medStatementpass').html(medStatementsuccesscount);
	          	$('#authsuccess').html('');
		        ...
		        stu3medicationStatementByPatientIdandInclude(strurl,data,stu3medStatementByPatientIdandinclude,stu3medStatementCollapseByPatientIdandinclude);
	          	stu3rendermedstatementresults(data,strurl,xhr,stu3medStatementByPatientId,stu3medStatementCollapseByPatientId,resType);
	          	l.ladda('stop');
	        },
	        error:function(e){
	        	medstatementtrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.stu3medStatementrunByPatId').html('Failed');
	        	$('.stu3medStatementrun').html('Failed');
	        	$('.stu3medStatementrun').parent().addClass('bg-danger');
	        	stu3rendermedstatementerror(e,stu3medStatementByPatientId);
	   		}
		})
	}

	stu3medicationStatementByPatientIdandInclude = function(strurl,data,stu3medStatementByPatientIdandinclude,stu3medStatementCollapseByPatientIdandinclude){
		var patientid = localStorage.getItem("patientid");
		l.ladda('start');
		access_token = localStorage.getItem("access_token");
		strurl = strurl+ "/MedicationStatement?patient="+patientid + "&_include=MedicationStatement:medication&_format=json";
		$.ajax({
      		url:strurl,
        	type:"GET",
	        beforeSend: function (xhr) {
	        ...
	        success:function(data,status,xhr){
	        	...
	        	if(data.entry){
	          	...
	          	medStatementsuccesscount++;
	          	$('.stu3medStatementrunByPatIdandinclude').html('Passed');
	          	if(medstatementtrigger == 0){
	          		$('.stu3medStatementrun').html('Passed');
	          	}
	          	$('.stu3medStatementpass').html(medStatementsuccesscount);
	          	stu3rendermedstatementresults(data,strurl,xhr,stu3medStatementByPatientIdandinclude,stu3medStatementCollapseByPatientIdandinclude,resType);
	        },
	        error:function(e){
	        	medstatementtrigger = 1;
	        	l.ladda( 'stop' );
	        	$('.stu3medStatementrunByPatIdandinclude').html('Failed');
	        	$('.stu3medStatementrun').html('Failed');
	        	$('.stu3medStatementrun').parent().addClass('bg-danger');
	        	stu3rendermedstatementerror(e,stu3medStatementByPatientIdandinclude);
	   		}
		})
	}

	stu3rendermedstatementresults = function(data,strurl,xhr,trid,colbyIdentifier,resType){
			/*$('.removabletr').remove();*/
			tr = $('<tr class="stu3medStatementremovabletr"></tr>');
	...
	stu3rendermedstatementerror = function(e,trid){
		var authfail;
		tr = $('<tr class="stu3medStatementremovabletr"></tr>');
		td = $('<td colspan="6" class="hiddenRow"></td>');
		if(e.status == '401'){
