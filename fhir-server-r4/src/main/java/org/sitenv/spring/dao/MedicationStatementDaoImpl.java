...
public class MedicationStatementDaoImpl extends AbstractDao implements MedicationStatementDao {
	...
	public DafMedicationStatement getMedicationStatementById(int id) {
		Criteria criteria = getSession().createCriteria(DafMedicationStatement.class).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY);
	...
	/**
	 * This method builds criteria for fetching particular version of the MedicationStatement record by id.
	 * @param theId : ID of the MedicationStatement
	 * @param versionId : version of the MedicationStatement record
	 * @return : DafMedicationStatement object
	 */
	@Override
            ...
            for (List<? extends IQueryParameterType> values : list) {
                ...
                for (IQueryParameterType params : values) {
                    ...
                    if (category.getValue() != null) {
                    	orCond = Restrictions.or(
                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->0->>'code' ilike '%" +category.getValue()+"%'"),
                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->0->>'display' ilike '%" +category.getValue()+"%'"),
                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->0->>'system' ilike '%" +category.getValue()+"%'"),

                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->1->>'code' ilike '%" +category.getValue()+"%'"),
                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->1->>'display' ilike '%" +category.getValue()+"%'"),
                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->1->>'system' ilike '%" +category.getValue()+"%'"),
                    				
                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->2->>'code' ilike '%" +category.getValue()+"%'"),
                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->2->>'display' ilike '%" +category.getValue()+"%'"),
                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->2->>'system' ilike '%" +category.getValue()+"%'"),

                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->3->>'code' ilike '%" +category.getValue()+"%'"),
                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->3->>'display' ilike '%" +category.getValue()+"%'"),
                    				Restrictions.sqlRestriction("{alias}.data->'category'->'coding'->3->>'system' ilike '%" +category.getValue()+"%'")
                    			);
                    } 
                    disjunction.add(orCond);
                }
    ...
    private void buildMedicationCriteria(SearchParameterMap theMap, Criteria criteria) {
	 ...
	 /**
  	 * This method builds criteria for source: Who or where the information in the statement came from
  	 * @param theMap : search parameter "source"
  	 * @param criteria : for retrieving entities by composing Criterion objects
  	 */
	   		...
	   		for (List<? extends IQueryParameterType> values : list) {
	   			...
	   			for (IQueryParameterType params : values) {
	                ...
	                if(source.getValue() != null){
	   					orCond = Restrictions.or(
	   								Restrictions.sqlRestriction("{alias}.data->'informationSource'->>'reference' ilike '%" + source.getValue() + "%'"),
	   								Restrictions.sqlRestriction("{alias}.data->'informationSource'->>'display' ilike '%" + source.getValue() + "%'"),
	   								Restrictions.sqlRestriction("{alias}.data->'informationSource'->>'type' ilike '%" + source.getValue() + "%'")
	   							);
	   				}else if(source.getMissing()){
	   					orCond = Restrictions.or(
	   								Restrictions.sqlRestriction("{alias}.data->>'informationSource' IS NULL")
	   							);
	   				}else if(!source.getMissing()){
	   					orCond = Restrictions.or(
	   								Restrictions.sqlRestriction("{alias}.data->>'informationSource' IS NOT NULL")
	   							);
		            }
	   				disjunction.add(orCond);
	   			}
	        ...
	        for (List<? extends IQueryParameterType> values : list) {
	            ...
	            for (IQueryParameterType params : values) {
	                ...
	                if (medicationCodeable.getValue() != null) {
	                	orCond = Restrictions.or(
	                    			Restrictions.sqlRestriction("{alias}.data->'medicationCodeableConcept'->'coding'->0->>'system' ilike '%" + medicationCodeable.getValue() + "%'"),
	                    			Restrictions.sqlRestriction("{alias}.data->'medicationCodeableConcept'->'coding'->0->>'code' ilike '%" + medicationCodeable.getValue() + "%'"),
	                    			Restrictions.sqlRestriction("{alias}.data->'medicationCodeableConcept'->'coding'->0->>'display' ilike '%" + medicationCodeable.getValue() + "%'"),
	                    			Restrictions.sqlRestriction("{alias}.data->'medicationCodeableConcept'->'coding'->1->>'system' ilike '%" + medicationCodeable.getValue() + "%'"),
	                    			Restrictions.sqlRestriction("{alias}.data->'medicationCodeableConcept'->'coding'->1->>'code' ilike '%" + medicationCodeable.getValue() + "%'"),
	                    			Restrictions.sqlRestriction("{alias}.data->'medicationCodeableConcept'->'coding'->1->>'display' ilike '%" + medicationCodeable.getValue() + "%'"),
	                    			Restrictions.sqlRestriction("{alias}.data->'medicationCodeableConcept'->'coding'->2->>'system' ilike '%" + medicationCodeable.getValue() + "%'"),
	                    			Restrictions.sqlRestriction("{alias}.data->'medicationCodeableConcept'->'coding'->2->>'code' ilike '%" + medicationCodeable.getValue() + "%'"),
	                    			Restrictions.sqlRestriction("{alias}.data->'medicationCodeableConcept'->'coding'->2->>'display' ilike '%" + medicationCodeable.getValue() + "%'"),
	                    			Restrictions.sqlRestriction("{alias}.data->'medicationCodeableConcept'->>'text' ilike '%" + medicationCodeable.getValue() + "%'")
	                			);
	                } 
	                disjunction.add(orCond);
	            }
   ...
   /**
     * This method builds criteria for fetching history of the MedicationStatement by id
     * @param theId : ID of the MedicationStatement
