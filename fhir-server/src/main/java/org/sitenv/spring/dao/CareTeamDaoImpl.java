...
public class CareTeamDaoImpl extends AbstractDao implements CareTeamDao {
    ...
    public List<DafCareTeam> getCareTeam(CareTeamSearchCriteria careTeamSearchCriteria) {
        ...
        if (careTeamSearchCriteria.getPatient() != null) {
            criteria.add(Restrictions.eq("careteam.patient.id", careTeamSearchCriteria.getPatient().intValue()));
        }

        if (StringUtils.isNotEmpty(careTeamSearchCriteria.getCat_code())) {
            criteria.add(Restrictions.eq("cat_code", careTeamSearchCriteria.getCat_code()));
        }

        if (StringUtils.isNotEmpty(careTeamSearchCriteria.getStatus())) {
            criteria.add(Expression.eq("status", careTeamSearchCriteria.getStatus()).ignoreCase());
        }
        return (List<DafCareTeam>) criteria.list();
    }
    ...
    public List<DafCareTeamParticipant> getCareteamparticipantByCareTeam(int id) {
        Criteria criteria = getSession().createCriteria(DafCareTeamParticipant.class, "dp")
                .add(Restrictions.eq("careteam", id));
        List<DafCareTeamParticipant> dafCareTeam = criteria.list();
        return dafCareTeam;
    }
